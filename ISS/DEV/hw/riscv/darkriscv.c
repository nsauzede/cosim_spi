...
static bool darkriscv_use_kvm_aia_aplic_imsic(RISCVVirtAIAType aia_type)
{
...
static bool darkriscv_use_emulated_aplic(RISCVVirtAIAType aia_type)
{
    bool msimode = aia_type == VIRT_AIA_TYPE_APLIC_IMSIC;
...
static bool darkriscv_aclint_allowed(void)
{
    return tcg_enabled() || qtest_enabled();
}

static const MemMapEntry darkriscv_memmap[] = {
    ...
    [DARKRISCV_SOC] =     { 0x40000000, DARKRISCV_SOC_MAX_SIZE },
    [VIRT_DRAM] =         { 0x80000000,           0x0 },
};

static void create_fdt_darkriscv(RISCVDarkriscvState *s, const MemMapEntry *memmap,
                            uint32_t irq_pcie_phandle,
                            uint32_t msi_pcie_phandle,
                            uint32_t iommu_sys_phandle)
{
    DeviceState *dev = qdev_new("darkriscv-soc");
    sysbus_realize_and_unref(SYS_BUS_DEVICE(dev), &error_fatal);
    sysbus_mmio_map(SYS_BUS_DEVICE(dev), 0, darkriscv_memmap[VIRT_PCIE_MMIO].base);
}
...
static MemMapEntry darkriscv_high_pcie_memmap;

#define VIRT_FLASH_SECTOR_SIZE (256 * KiB)

static PFlashCFI01 *darkriscv_flash_create1(RISCVDarkriscvState *s,
                                       const char *name,
                                       const char *alias_prop_name)
{
    /*
     * Create a single flash device.  We use the same parameters as
     * the flash devices on the ARM darkriscv board.
     */
...
static void darkriscv_flash_create(RISCVDarkriscvState *s)
{
    s->flash[0] = darkriscv_flash_create1(s, "darkriscv.flash0", "pflash0");
    s->flash[1] = darkriscv_flash_create1(s, "darkriscv.flash1", "pflash1");
}

static void darkriscv_flash_map1(PFlashCFI01 *flash,
...
static void darkriscv_flash_map(RISCVDarkriscvState *s,
                           MemoryRegion *sysmem)
{
    hwaddr flashsize = darkriscv_memmap[VIRT_FLASH].size / 2;
    hwaddr flashbase = darkriscv_memmap[VIRT_FLASH].base;

    darkriscv_flash_map1(s->flash[0], flashbase, flashsize,
                    sysmem);
    darkriscv_flash_map1(s->flash[1], flashbase + flashsize, flashsize,
                    sysmem);
}

#ifndef DARKRISCV
static void create_pcie_irq_map(RISCVDarkriscvState *s, void *fdt, char *nodename,
...
static void create_fdt_socket_cpus(RISCVDarkriscvState *s, int socket,
...
static void create_fdt_socket_memory(RISCVDarkriscvState *s,
...
static void create_fdt_socket_clint(RISCVDarkriscvState *s,
                                    const MemMapEntry *memmap, int socket,
                                    uint32_t *intc_phandles)
{
    int cpu;
...
static void create_fdt_socket_aclint(RISCVDarkriscvState *s,
                                     const MemMapEntry *memmap, int socket,
                                     uint32_t *intc_phandles)
{
    int cpu;
...
static void create_fdt_socket_plic(RISCVDarkriscvState *s,
                                   const MemMapEntry *memmap, int socket,
...
static void create_fdt_one_imsic(RISCVDarkriscvState *s, hwaddr base_addr,
...
static void create_fdt_imsic(RISCVDarkriscvState *s, const MemMapEntry *memmap,
                             uint32_t *phandle, uint32_t *intc_phandles,
...
static void create_fdt_one_aplic(RISCVDarkriscvState *s, int socket,
...
static void create_fdt_socket_aplic(RISCVDarkriscvState *s,
                                    const MemMapEntry *memmap, int socket,
...
static void create_fdt_pmu(RISCVDarkriscvState *s)
{
...
static void create_fdt_sockets(RISCVDarkriscvState *s, const MemMapEntry *memmap,
                               uint32_t *phandle,
                               uint32_t *irq_mmio_phandle,
                               uint32_t *irq_pcie_phandle,
                               uint32_t *irq_darkriscvio_phandle,
                               uint32_t *msi_pcie_phandle)
{
    ...
    for (socket = (socket_count - 1); socket >= 0; socket--) {
        ...
        if (darkriscv_aclint_allowed() && s->have_aclint) {
     ...
     */
    if (!darkriscv_use_emulated_aplic(s->aia_type)) {
        ...
        *irq_darkriscvio_phandle = xplic_phandles[0];
        *irq_pcie_phandle = xplic_phandles[0];
        ...
        for (socket = 0; socket < socket_count; socket++) {
            if (socket == 0) {
                ...
                *irq_darkriscvio_phandle = xplic_phandles[socket];
...
static void create_fdt_darkriscvio(RISCVDarkriscvState *s, const MemMapEntry *memmap,
                              uint32_t irq_darkriscvio_phandle)
{
    int i;
    MachineState *ms = MACHINE(s);
    ...
    for (i = 0; i < VIRTIO_COUNT; i++) {
        g_autofree char *name =  g_strdup_printf("/soc/darkriscvio_mmio@%lx",
            (long)(memmap[VIRT_VIRTIO].base + i * memmap[VIRT_VIRTIO].size));

        qemu_fdt_add_subnode(ms->fdt, name);
        qemu_fdt_setprop_string(ms->fdt, name, "compatible", "darkriscvio,mmio");
        qemu_fdt_setprop_cells(ms->fdt, name, "reg",
            0x0, memmap[VIRT_VIRTIO].base + i * memmap[VIRT_VIRTIO].size,
            0x0, memmap[VIRT_VIRTIO].size);
        qemu_fdt_setprop_cell(ms->fdt, name, "interrupt-parent",
            irq_darkriscvio_phandle);
        if (s->aia_type == VIRT_AIA_TYPE_NONE) {
...
#ifndef DARKRISCV
static void create_fdt_pcie(RISCVDarkriscvState *s, const MemMapEntry *memmap,
                            uint32_t irq_pcie_phandle,
                            uint32_t msi_pcie_phandle,
                            uint32_t iommu_sys_phandle)
{
    ...
    qemu_fdt_setprop_sized_cells(ms->fdt, name, "ranges",
        ...
        2, darkriscv_high_pcie_memmap.base,
        2, darkriscv_high_pcie_memmap.base, 2, darkriscv_high_pcie_memmap.size);

    if (darkriscv_is_iommu_sys_enabled(s)) {
...
#endif
static void create_fdt_reset(RISCVDarkriscvState *s, const MemMapEntry *memmap,
                             uint32_t *phandle)
{
    char *name;
...
static void create_fdt_uart(RISCVDarkriscvState *s, const MemMapEntry *memmap,
                            uint32_t irq_mmio_phandle)
{
    g_autofree char *name = NULL;
    MachineState *ms = MACHINE(s);
...
static void create_fdt_rtc(RISCVDarkriscvState *s, const MemMapEntry *memmap,
                           uint32_t irq_mmio_phandle)
{
    g_autofree char *name = NULL;
    MachineState *ms = MACHINE(s);
...
static void create_fdt_flash(RISCVDarkriscvState *s, const MemMapEntry *memmap)
{
    MachineState *ms = MACHINE(s);
    hwaddr flashsize = darkriscv_memmap[VIRT_FLASH].size / 2;
    hwaddr flashbase = darkriscv_memmap[VIRT_FLASH].base;
...
static void create_fdt_fw_cfg(RISCVDarkriscvState *s, const MemMapEntry *memmap)
{
    MachineState *ms = MACHINE(s);
...
static void create_fdt_darkriscvio_iommu(RISCVDarkriscvState *s, uint16_t bdf)
{
    const char compat[] = "darkriscvio,pci-iommu\0pci1af4,1057";
    ...
    pci_node = g_strdup_printf("/soc/pci@%lx",
                               (long) darkriscv_memmap[VIRT_PCIE_ECAM].base);
    iommu_node = g_strdup_printf("%s/darkriscvio_iommu@%x,%x", pci_node,
...
#ifndef DARKRISCV
static void create_fdt_iommu_sys(RISCVDarkriscvState *s, uint32_t irq_chip,
                                 uint32_t msi_phandle,
                                 uint32_t *iommu_sys_phandle)
{
...
static void create_fdt_iommu(RISCVDarkriscvState *s, uint16_t bdf)
{
...
static void finalize_fdt(RISCVDarkriscvState *s)
{
    uint32_t phandle = 1, irq_mmio_phandle = 1, msi_pcie_phandle = 1;
    uint32_t irq_pcie_phandle = 1, irq_darkriscvio_phandle = 1;
    uint32_t iommu_sys_phandle = 1;

    create_fdt_sockets(s, darkriscv_memmap, &phandle, &irq_mmio_phandle,
                       &irq_pcie_phandle, &irq_darkriscvio_phandle,
                       &msi_pcie_phandle);

    create_fdt_darkriscvio(s, darkriscv_memmap, irq_darkriscvio_phandle);
...
#ifdef DARKRISCV
    create_fdt_darkriscv(s, darkriscv_memmap, irq_pcie_phandle, msi_pcie_phandle,
            iommu_sys_phandle);
#else
    create_fdt_pcie(s, darkriscv_memmap, irq_pcie_phandle, msi_pcie_phandle,
                    iommu_sys_phandle);
#endif

    create_fdt_reset(s, darkriscv_memmap, &phandle);

    create_fdt_uart(s, darkriscv_memmap, irq_mmio_phandle);

    create_fdt_rtc(s, darkriscv_memmap, irq_mmio_phandle);
}

static void create_fdt(RISCVDarkriscvState *s, const MemMapEntry *memmap)
{
    MachineState *ms = MACHINE(s);
    uint8_t rng_seed[32];
    g_autofree char *name = NULL;
    ...
    qemu_fdt_setprop_string(ms->fdt, "/", "model", "riscv-darkriscvio,qemu");
    qemu_fdt_setprop_string(ms->fdt, "/", "compatible", "riscv-darkriscvio");
                                          ...
                                          RISCVDarkriscvState *s)
{
    ...
    hwaddr high_mmio_base = darkriscv_high_pcie_memmap.base;
    hwaddr high_mmio_size = darkriscv_high_pcie_memmap.size;
    ...
    /* Set GPEX object properties for the darkriscv machine */
...
{
    hwaddr base = darkriscv_memmap[VIRT_FW_CFG].base;
    FWCfgState *fw_cfg;
...
static DeviceState *darkriscv_create_plic(const MemMapEntry *memmap, int socket,
...
static DeviceState *darkriscv_create_aia(RISCVVirtAIAType aia_type, int aia_guests,
                                    const MemMapEntry *memmap, int socket,
                                    int base_hartid, int hart_count)
{
    int i;
...
static void create_platform_bus(RISCVDarkriscvState *s, DeviceState *irqchip)
{
    DeviceState *dev;
    SysBusDevice *sysbus;
    const MemMapEntry *memmap = darkriscv_memmap;
    int i;
...
static void darkriscv_build_smbios(RISCVDarkriscvState *s)
{
...
static void darkriscv_machine_done(Notifier *notifier, void *data)
{
    RISCVDarkriscvState *s = container_of(notifier, RISCVDarkriscvState,
                                     machine_done);
    const MemMapEntry *memmap = darkriscv_memmap;
    ...
    if (pflash_blk0) {
        if (machine->firmware && !strcmp(machine->firmware, "none") &&
             ...
             */
            start_addr = darkriscv_memmap[VIRT_FLASH].base;
        } else {
            /*
            ...
            kernel_entry = darkriscv_memmap[VIRT_FLASH].base;
        }
    }
    ...
    riscv_setup_rom_reset_vec(machine, &s->soc[0], start_addr,
                              darkriscv_memmap[VIRT_MROM].base,
                              darkriscv_memmap[VIRT_MROM].size, kernel_entry,
                              fdt_load_addr);
    ...
    darkriscv_build_smbios(s);

#ifndef DARKRISCV
    if (darkriscv_is_acpi_enabled(s)) {
        darkriscv_acpi_setup(s);
    }
#endif
}

static void darkriscv_machine_init(MachineState *machine)
{
    const MemMapEntry *memmap = darkriscv_memmap;
    RISCVDarkriscvState *s = RISCV_VIRT_MACHINE(machine);
    MemoryRegion *system_memory = get_system_memory();
    MemoryRegion *mask_rom = g_new(MemoryRegion, 1);
    DeviceState *mmio_irqchip, *darkriscvio_irqchip, *pcie_irqchip;
    ...
    /* Initialize sockets */
    mmio_irqchip = darkriscvio_irqchip = pcie_irqchip = NULL;
    for (i = 0; i < socket_count; i++) {
        ...
        if (s->aia_type == VIRT_AIA_TYPE_NONE) {
            s->irqchip[i] = darkriscv_create_plic(memmap, i,
                                             base_hartid, hart_count);
        } else {
            s->irqchip[i] = darkriscv_create_aia(s->aia_type, s->aia_guests,
        ...
        if (i == 0) {
            ...
            darkriscvio_irqchip = s->irqchip[i];
    ...
    if (kvm_enabled() && darkriscv_use_kvm_aia_aplic_imsic(s->aia_type)) {
...
#endif
        darkriscv_high_pcie_memmap.base = VIRT32_HIGH_PCIE_MMIO_BASE;
        darkriscv_high_pcie_memmap.size = VIRT32_HIGH_PCIE_MMIO_SIZE;
    } else {
        darkriscv_high_pcie_memmap.size = VIRT64_HIGH_PCIE_MMIO_SIZE;
        darkriscv_high_pcie_memmap.base = memmap[VIRT_DRAM].base + machine->ram_size;
        darkriscv_high_pcie_memmap.base =
            ROUND_UP(darkriscv_high_pcie_memmap.base, darkriscv_high_pcie_memmap.size);
    }

    s->memmap = darkriscv_memmap;
    ...
    /* boot rom */
    memory_region_init_rom(mask_rom, NULL, "riscv_darkriscv_board.mrom",
    ...
    /* VirtIO MMIO devices */
    for (i = 0; i < VIRTIO_COUNT; i++) {
        sysbus_create_simple("darkriscvio-mmio",
            memmap[VIRT_VIRTIO].base + i * memmap[VIRT_VIRTIO].size,
            qdev_get_gpio_in(darkriscvio_irqchip, VIRTIO_IRQ + i));
    }
    ...
    darkriscv_flash_map(s, system_memory);
    ...
    s->machine_done.notify = darkriscv_machine_done;
    qemu_add_machine_init_done_notifier(&s->machine_done);
}

static void darkriscv_machine_instance_init(Object *obj)
{
    RISCVDarkriscvState *s = RISCV_VIRT_MACHINE(obj);

    darkriscv_flash_create(s);
...
static char *darkriscv_get_aia_guests(Object *obj, Error **errp)
{
    RISCVDarkriscvState *s = RISCV_VIRT_MACHINE(obj);
...
static void darkriscv_set_aia_guests(Object *obj, const char *val, Error **errp)
{
    RISCVDarkriscvState *s = RISCV_VIRT_MACHINE(obj);
...
static char *darkriscv_get_aia(Object *obj, Error **errp)
{
    RISCVDarkriscvState *s = RISCV_VIRT_MACHINE(obj);
    const char *val;
...
static void darkriscv_set_aia(Object *obj, const char *val, Error **errp)
{
    RISCVDarkriscvState *s = RISCV_VIRT_MACHINE(obj);
...
static bool darkriscv_get_aclint(Object *obj, Error **errp)
{
    RISCVDarkriscvState *s = RISCV_VIRT_MACHINE(obj);
...
static void darkriscv_set_aclint(Object *obj, bool value, Error **errp)
{
    RISCVDarkriscvState *s = RISCV_VIRT_MACHINE(obj);
...
#ifndef DARKRISCV
bool darkriscv_is_iommu_sys_enabled(RISCVDarkriscvState *s)
{
    return s->iommu_sys == ON_OFF_AUTO_ON;
}
#endif

static void darkriscv_get_iommu_sys(Object *obj, Visitor *v, const char *name,
...
static void darkriscv_set_iommu_sys(Object *obj, Visitor *v, const char *name,
                               void *opaque, Error **errp)
{
    RISCVDarkriscvState *s = RISCV_VIRT_MACHINE(obj);
...
#ifndef DARKRISCV
bool darkriscv_is_acpi_enabled(RISCVDarkriscvState *s)
{
    return s->acpi != ON_OFF_AUTO_OFF;
}
#endif

static void darkriscv_get_acpi(Object *obj, Visitor *v, const char *name,
                          void *opaque, Error **errp)
{
    RISCVDarkriscvState *s = RISCV_VIRT_MACHINE(obj);
...
static void darkriscv_set_acpi(Object *obj, Visitor *v, const char *name,
                          void *opaque, Error **errp)
{
    RISCVDarkriscvState *s = RISCV_VIRT_MACHINE(obj);
...
static HotplugHandler *darkriscv_machine_get_hotplug_handler(MachineState *machine,
                                                        DeviceState *dev)
{
...
static void darkriscv_machine_device_plug_cb(HotplugHandler *hotplug_dev,
                                        DeviceState *dev, Error **errp)
{
    RISCVDarkriscvState *s = RISCV_VIRT_MACHINE(hotplug_dev);
    ...
    if (object_dynamic_cast(OBJECT(dev), TYPE_VIRTIO_IOMMU_PCI)) {
        create_fdt_darkriscvio_iommu(s, pci_get_bdf(PCI_DEVICE(dev)));
    }
...
static void darkriscv_machine_class_init(ObjectClass *oc, void *data)
{
    MachineClass *mc = MACHINE_CLASS(oc);
    HotplugHandlerClass *hc = HOTPLUG_HANDLER_CLASS(oc);

    mc->desc = "RISC-V VirtIO board";
    mc->init = darkriscv_machine_init;
    ...
    mc->default_ram_id = "riscv_darkriscv_board.ram";
    assert(!mc->get_hotplug_handler);
    mc->get_hotplug_handler = darkriscv_machine_get_hotplug_handler;

    hc->plug = darkriscv_machine_device_plug_cb;
    ...
    object_class_property_add_bool(oc, "aclint", darkriscv_get_aclint,
                                   darkriscv_set_aclint);
    object_class_property_set_description(oc, "aclint",
                                          "(TCG only) Set on/off to "
                                          "enable/disable emulating "
                                          "ACLINT devices");

    object_class_property_add_str(oc, "aia", darkriscv_get_aia,
                                  darkriscv_set_aia);
    ...
    object_class_property_add_str(oc, "aia-guests",
                                  darkriscv_get_aia_guests,
                                  darkriscv_set_aia_guests);
    {
        g_autofree char *str =
    ...
    object_class_property_add(oc, "acpi", "OnOffAuto",
                              darkriscv_get_acpi, darkriscv_set_acpi,
                              NULL, NULL);
    object_class_property_set_description(oc, "acpi",
                                          "Enable ACPI");

    object_class_property_add(oc, "iommu-sys", "OnOffAuto",
                              darkriscv_get_iommu_sys, darkriscv_set_iommu_sys,
                              NULL, NULL);
    object_class_property_set_description(oc, "iommu-sys",
                                          "Enable IOMMU platform device");
}

static const TypeInfo darkriscv_machine_typeinfo = {
    .name       = MACHINE_TYPE_NAME("darkriscv"),
    .parent     = TYPE_MACHINE,
    .class_init = darkriscv_machine_class_init,
    .instance_init = darkriscv_machine_instance_init,
    .instance_size = sizeof(RISCVDarkriscvState),
    .interfaces = (InterfaceInfo[]) {
         { TYPE_HOTPLUG_HANDLER },
         { }
    },
};
static void darkriscv_machine_init_register_types(void)
{
    printf("%s:\n", __func__);
    type_register_static(&darkriscv_machine_typeinfo);
}

type_init(darkriscv_machine_init_register_types)
